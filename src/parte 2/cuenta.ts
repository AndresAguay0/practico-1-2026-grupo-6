...
export class Cuenta {
    ...
    depositar(monto: number): void {
        ...
        if (monto < 0) {
            throw new Error("El monto a depositar no debe ser un número negativo.");
        }
    ...
    retirar(monto: number): void {
        ...
        if (monto < 0) {
            throw new Error("El monto a retirar no debe ser un número negativo.");
        }
        if (monto > this.saldo) {
            throw new Error("El monto a retirar no debe más alto que el saldo original.");
        }
